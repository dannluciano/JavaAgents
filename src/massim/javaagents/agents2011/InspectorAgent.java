...
public class InspectorAgent extends Agent {
		...
		for (Percept p : percepts) {
			if (p.getName().equals("step")) {
				...
				if (health.intValue() <= 1) {
					...
					// seria interessante mandar mensagem para o reparador do
					// proprio time
	...
	private Action planInspect() {
		...
		println("there are " + adjacentNum
				+ " visible opponents that I could inspect");
		/*
		 * Como tem inimigos no vertice, eu chamarei os sabotadores para atacar
		 * O sabotador tem que ver no handleBeliefs se ele tiver uma cren�a de
		 * inimigo, ele deve atacar
		 */
		if (enemies != null) {
